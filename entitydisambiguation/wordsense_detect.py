...
class MultiSenDetect(object):
    ...
    def collect_mutilsens(self, word):
        ...
        sens = [''.join(i.split('：')[1:]) for i in
                selector.xpath('//li[@class="list-dot list-dot-paddingleft"]/div/a/text()')]
        sens_link = ['http://baike.baidu.com' + i for i in
                     selector.xpath('//li[@class="list-dot list-dot-paddingleft"]/div/a/@href')]
    ...
    def distance_words(self, sent1, sent2):
        ...
        for word1 in wds1:
            score = max(
                [self.similarity_cosine(self.get_wordvector(word1), self.get_wordvector(word2)) for word2 in wds2])
            score_wds1.append(score)
        for word2 in wds2:
            score = max(
                [self.similarity_cosine(self.get_wordvector(word2), self.get_wordvector(word1)) for word1 in wds1])
